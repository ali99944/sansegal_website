...
export function FloatingWhatsApp({ dictionary }: TranslatedViewProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute end-16 whitespace-nowrap rounded-md bg-black/80 px-3 py-2 text-sm font-semibold text-white hidden transition-all duration-300 group-hover:block group-hover:end-20">
        {whatsAppDictionary.ariaLabel}
        <div className="absolute end-[-4px] top-1/2 -translate-y-1/2 h-2 w-2 rotate-45 bg-black/80" />
      </div>

      {/* Main Floating Button */}
      <div className="flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg transition-all duration-300 hover:bg-[#20b358] ">
